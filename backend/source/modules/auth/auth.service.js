...
export const verifyEmail = async (payload) => {
  ...
  const record = await prisma.otpVerification.findFirst({
    where: { email, purpose: "EMAIL_VERIFICATION", verifiedAt: null },
    orderBy: { createdAt: "desc" },
  });
  if (!record || record.expiresAt < new Date())
    throw new ApiError(400, "OTP has expired or is invalid.");
  if (!(await bcrypt.compare(String(payload.otp).trim(), record.codeHash)))
...
const createTrustedDevice = async (user, userAgent, ipAddress) => {
  ...
  await prisma.trustedDevice.create({
    data: {
      ...
      userAgent,
      ipAddress,
      expiresAt: new Date(Date.now() + TRUSTED_DEVICE_MAX_AGE),
    },
  });
  return token;
};
...
export const verifyLogin = async (payload) => {
  ...
  if (!user || !user.emailVerified || user.status !== "ACTIVE")
    throw new ApiError(401, "Your login session is no longer valid.");
  const record = await prisma.otpVerification.findFirst({
    where: { userId: user.id, email, purpose: "LOGIN", verifiedAt: null },
    orderBy: { createdAt: "desc" },
  });
  if (!record || record.expiresAt < new Date())
    throw new ApiError(400, "OTP has expired or is invalid.");
  ...
  if (payload.trustDevice)
    tokens.trustedDeviceToken = await createTrustedDevice(
      user,
      payload.userAgent,
      payload.ipAddress,
    );
